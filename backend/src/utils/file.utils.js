...
const { resolveFromBackend } = require("./project-paths");

function ensureDirectoryExists(relativePath) {
  const fullPath = resolveFromBackend(relativePath);
...
function readFilesRecursively(rootPath, ignoredDirectories = new Set()) {
  ...
  function walk(currentPath) {
    ...
    entries.forEach((entry) => {
      ...
      if (/\.(js|cjs|mjs|ts|json|java|xml|gradle|properties|yml|yaml)$/i.test(entry.name)) {

...
module.exports = [
  createRule({
    ...
    evaluate: (scanResult) => {
      ...
      return buildFailedRule({
        weight: 25,
        message: `No explicit global error middleware was detected for ${scanResult.apis.length} route(s).`,
        issues: [buildIssue({
          category: "reliability",
          severity: "high",
  ...
  createRule({
    ...
    evaluate: (scanResult) => {
      ...
      return buildFailedRule({
        weight: 15,
        message: `Only ${scanResult.tryCatchCount} defensive handler(s) were found for ${apiCount} routes, or ${Math.round(tryCatchCoverage * 100)}% coverage.`,
        issues: [buildIssue({
          category: "reliability",
          severity: "medium",
  ...
  createRule({
    ...
    evaluate: (scanResult) => {
      ...
      return buildFailedRule({
        weight: 8,
        impact: Math.min(18, 8 + scanResult.externalCallSignals.length * 2),
        message: `Retry or timeout logic is missing for ${scanResult.externalCallSignals.length} external-call file(s).`,
        issues: [buildIssue({
          category: "reliability",
          severity: "low",

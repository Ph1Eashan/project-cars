...
function createEmptyRepositoryZip() {
  const archive = new AdmZip();
  archive.addFile("empty-repo/", Buffer.alloc(0));

  const zipPath = path.join(os.tmpdir(), `project-cars-empty-fixture-${Date.now()}.zip`);
  archive.writeZip(zipPath);
  return zipPath;
}
